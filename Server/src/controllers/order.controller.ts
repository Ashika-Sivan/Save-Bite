...
export class OrderController {
    ...
    createCheckout = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    getOrderById = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const customerId = req.user?.userId
    ...
    getMyOrders = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const customerId = req.user?.userId
        if (!customerId) {
            throw new AppError(AUTH_MESSAGES.USER_NOT_AUTHENTICATED, StatusCode.UNAUTHORIZED)
        }
    ...
    verifyPayment = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const customerId = req.user?.userId
        const orderId = req.params.orderId
        if (!customerId) {
            throw new AppError(AUTH_MESSAGES.USER_NOT_AUTHENTICATED, StatusCode.UNAUTHORIZED)
    ...
    handleWebhook = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    cancelOrder = catchAsync(async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
        const customerId = req.user?.userId;
        const orderId = req.params.orderId;
        if (!customerId) {
            throw new AppError(AUTH_MESSAGES.USER_NOT_AUTHENTICATED, StatusCode.UNAUTHORIZED);
