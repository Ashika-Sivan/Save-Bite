...
export class EmailService implements IEmailService {
  async sendOtpEmail(email: string, otp: string): Promise<void> {
    try {
      await transporter.sendMail({
        from: `"SaveBite" <${env.EMAIL_USER}>`,
        to: email,
        subject: "Your SaveBite verification code",
        html: this._otpTemplate(otp),
      });
      Logger.info("OTP email sent successfully");
    } catch (error: unknown) {
      const err = error as Error;
      Logger.error(`[Email Error] Failed to send OTP email to ${email}:`, err.message);
      throw error;
    }
  }
  ...
  async sendResetPasswordEmail(email: string, token: string): Promise<void> {
    ...
    try {
      await transporter.sendMail({
        ...
        html: this._resetPasswordTemplate(resetLink),
      });
      Logger.info("Reset password email sent successfully");
    } catch (error: unknown) {
      const err = error as Error;
      Logger.error(`[Email Error] Failed to send reset email to ${email}:`, err.message);
      throw error;
    }
  }

  private _otpTemplate(otp: string): string {
    return `
    <!DOCTYPE html>
    <html>
      <head>
  ...
  private _resetPasswordTemplate(resetLink: string): string {
    return `
    <!DOCTYPE html>
    <html>
      <head>
        <meta charset="utf-8" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <title>Reset your SaveBite password</title>
      </head>
      <body style="margin:0;padding:0;background-color:#f4f7f2;font-family:'Helvetica Neue',Arial,sans-serif;color:#1f2d24;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f7f2;padding:40px 16px;">
          <tr>
                ...
                <tr>
                  <td style="padding:36px 32px 8px 32px;">
                    ...
                    <div style="text-align:center;margin:8px 0 28px 0;">
                      <a href="${resetLink}" style="display:inline-block;background:#2f7d4f;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:14px 28px;border-radius:10px;">
                        Reset Password
                      </a>
                    </div>
